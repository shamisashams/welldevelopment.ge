...
export const SelectFilter = ({ icon, title, subtitle, children }) => {
  ...
  return (
    <div className="select relative xl:border-r xl:px-5 md:px-3 mx-1  xl:mb-0 mb-2 2xl:text-base text-sm">
      ...
      <div
        className={`xl:absolute xl:left-0 xl:top-24 rounded-xl bg-white xl:shadow-lg transition-all duration-300 xl:py-4 xl:px-6 w-full text-sm z-40 overflow-hidden ${
          clicked
...
export const MainButton = ({ text, onClick }) => {
  return (
    <button onClick={onClick} className="bg-custom-blue shadow-lg shadow-custom-blue/[0.5] text-white py-3 px-8 rounded-lg">
      {text}
    </button>
  );
};
